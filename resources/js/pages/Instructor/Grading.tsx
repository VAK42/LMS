      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-black dark:text-white">Grading</h1>
              <p className="text-zinc-600 dark:text-zinc-400">{pendingCount} Pending • {gradedCount} Graded</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
            <div className="p-4 border-b border-zinc-200 dark:border-zinc-700">
              <h2 className="font-bold text-black dark:text-white">Submissions</h2>
            </div>
          ...
          <div className="lg:col-span-2">
            {selectedSubmission ? (
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
                <div className="mb-6">
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${selectedSubmission.score !== null ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400'}`}>
                      {selectedSubmission.score !== null ? 'Graded' : 'Pending'}
                    </span>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-zinc-50 dark:bg-zinc-800 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-black dark:text-white">{selectedSubmission.assessment.lesson?.module?.course?.courseTitle || 'N/A'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-sm text-zinc-500">Passing Score</p>
                    <p className="font-medium text-black dark:text-white">{Math.round(selectedSubmission.assessment.passingScore)}%</p>
                  </div>
                </div>
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Score [0 - 100]</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Feedback</label>
                    <textarea value={feedback} onChange={e => setFeedback(e.target.value)} rows={4} className="w-full px-4 py-3 rounded-lg border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Provide Feedback For The Student..." />
                  </div>
                  <div className="flex justify-end gap-4">
                    <button onClick={() => setSelectedSubmission(null)} className="px-6 py-2 rounded-lg text-zinc-600 dark:text-zinc-300 font-medium hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer">Cancel</button>
                    <button onClick={handleSaveGrade} disabled={saving} className="px-6 py-2 rounded-lg text-green-600 border-green-600 border font-medium hover:bg-green-900 hover:text-white disabled:opacity-50 cursor-pointer">{saving ? 'Saving...' : 'Save Grade'}</button>
                  </div>
                </div>
              </div>
            ) : (
              <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-12 text-center">
                ...
                <h3 className="text-xl font-bold text-black dark:text-white mb-2">Select A Submission</h3>
                <p className="text-zinc-600 dark:text-zinc-400">Choose A Student Submission From The List To Start Grading</p>
              </div>
            )}
          </div>
        </div>
