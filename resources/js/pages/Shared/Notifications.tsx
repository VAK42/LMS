    ...
    <Layout user={user}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-serif font-bold text-black dark:text-white">Notifications</h1>
        ...
        <div className="space-y-3">
            ...
            <div key={notification.notificationId} className={`p-4 border transition-all ${notification.isRead ? 'bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800' : 'bg-zinc-50 dark:bg-zinc-800 border-zinc-300 dark:border-zinc-700'}`}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex gap-2">
                  {!notification.isRead && (
                    <button onClick={() => handleMarkAsRead(notification.notificationId)} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer" title="Mark As Read">
                      <CheckCheck className="w-5 h-5 text-zinc-600 dark:text-zinc-400" />
                    </button>
                  )}
                  <button onClick={() => handleDelete(notification.notificationId)} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer" title="Delete">
          ...
          {notifications.data.length === 0 && (
            <div className="text-center py-16 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800">
              ...
              <h3 className="text-xl font-bold text-black dark:text-white mb-2">No Notifications</h3>
