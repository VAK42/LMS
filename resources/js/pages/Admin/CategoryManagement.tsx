...
export default function CategoryManagement({ categories, filters, user }: Props) {
  ...
  const handleSearch = () => {
    router.get('/admin/categories', { search: searchTerm }, { preserveState: true });
  };
  ...
  const handleEdit = (data: Record<string, any>) => {
    ...
    router.put(`/admin/categories/${selectedCategory.categoryId}`, data, {
      onSuccess: () => {
  ...
  const handleDelete = (category: Category) => {
    if (category.courses_count > 0) {
      alert(`Cannot Delete Category With ${category.courses_count} Courses!`);
      return;
    }
    if (confirm('Are You Sure You Want To Delete This Category?')) {
      router.delete(`/admin/categories/${category.categoryId}`);
    }
  };
    ...
    },
    {
      ...
      render: (_: any, row: Category) => (
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => handleDelete(row)} disabled={row.courses_count > 0} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-red-600 disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      <div className="flex bg-zinc-50 dark:bg-black min-h-screen">
        ...
        <div className="flex-1">
          <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <DataTable columns={columns} data={categories.data} searchable={false} exportable={true} />
