...
export default function UserManagement({ users, filters, user }: Props) {
  ...
  const handleEdit = (data: Record<string, any>) => {
    ...
    router.put(`/admin/users/${selectedUser.userId}`, data, {
      onSuccess: () => {
    ...
    },
    {
      ...
      render: (_: any, row: User) => (
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => handleDelete(row.userId)} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 text-red-600">
          ...
          <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 mb-6">
              <div className="flex items-center gap-4">
                ...
                <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="px-4 py-2 border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:border-black dark:focus:border-white">
                  <option value="">All Roles</option>
                  <option value="learner">Learner</option>
                  <option value="instructor">Instructor</option>
                  <option value="admin">Admin</option>
                </select>
                <button onClick={handleSearch} className="flex items-center gap-2 px-6 py-2 bg-black dark:bg-white text-white dark:text-black hover:bg-zinc-800 dark:hover:bg-zinc-200">
                  <Filter className="w-4 h-4" />
                  Filter
                </button>
              </div>
            </div>
            <DataTable columns={columns} data={users.data} searchable={false} exportable={true} />
