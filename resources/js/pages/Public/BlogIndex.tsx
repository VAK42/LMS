      ...
      <div className="bg-white dark:bg-black min-h-screen py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h1 className="text-4xl text-black dark:text-white sm:text-5xl">{t('latestArticles')}</h1>
            ...
            <div className="flex items-center gap-4">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-black dark:focus:ring-white transition-shadow"
                />
              </div>
              <button
                onClick={handleSearch}
                className="px-8 py-3 rounded bg-black dark:bg-white text-white dark:text-black font-bold hover:opacity-90 transition-opacity cursor-pointer"
              >
                {t('searchButton')}
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            {blogs.data.map((blog) => (
              <div key={blog.blogId} className="flex flex-col bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 hover:border-black dark:hover:border-white transition-colors duration-300 overflow-hidden group">
                <div className="flex-shrink-0 relative h-48 w-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center overflow-hidden">
                  {blog.thumbnail ? (
                ...
                <div className="flex-1 p-6 flex flex-col justify-between">
                  <div className="flex-1">
                    ...
                    <Link href={`/blogs/${blog.slug}`} className="block mt-2">
                      <h3 className="text-xl font-semibold text-black dark:text-white hover:underline transition-all line-clamp-2">{blog.title}</h3>
                  ...
                  <div className="mt-6 flex items-center">
                    <div className="flex-shrink-0">
                      ...
                      <div className="h-10 w-10 bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center border border-zinc-200 dark:border-zinc-700 rounded-full">
                        <User className="h-6 w-6 text-zinc-500 dark:text-zinc-400" />
                      </div>
                    </div>
                    <div className="ml-3">
                      <p className="text-sm font-medium text-black dark:text-white">{blog.instructor.userName}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {blogs.last_page > 1 && (
            <div className="flex justify-center items-center gap-2">
              <button
                onClick={() => router.get('/blogs', buildPaginationParams(1), { preserveState: true })}
              ...
              <button
                onClick={() => router.get('/blogs', buildPaginationParams(blogs.current_page - 1), { preserveState: true })}
                disabled={blogs.current_page === 1}
                className="px-3 py-2 border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:border-black dark:hover:border-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
              ...
              <button
                onClick={() => router.get('/blogs', buildPaginationParams(blogs.last_page), { preserveState: true })}
                disabled={blogs.current_page === blogs.last_page}
                className="px-3 py-2 border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 hover:border-black dark:hover:border-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
          ...
          {blogs.data.length === 0 && (
            <div className="text-center py-20">
              ...
              <h3 className="text-xl font-semibold text-black dark:text-white mb-2">{t('noBlogsFound')}</h3>
